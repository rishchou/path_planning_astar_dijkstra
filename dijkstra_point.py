                ...
                if i != 0 or j != 0:
                    ...
                    if graph[neighbour]['valid'] == True:
                        ...
                        if graph[neighbour]['visited'] == False:
                            ...
                            heappush(queue, (graph[neighbour]['distance'], neighbour))
                        # else:
                        #     if graph[neighbour]['distance'] > graph[current]['distance'] + distance:
                        #         graph[neighbour]['distance'] = graph[current]['distance'] + distance
                        #         graph[neighbour]['parent'] = current
                        #         heappush(queue, (graph[neighbour]['distance'], neighbour))		
    path = [(goal_x, goal_y)]
...
if __name__ == "__main__":
    ...
    plt.plot(x1,y1,'Xr')
    plt.plot(p,q,'Xg')   
    min_distance, path = dijkstra(g, start, goal)
